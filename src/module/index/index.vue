<template>
    ...
    <el-header style="height:auto" id="head">
      <el-row>
        ...
        <li><span class="iconfont icon-set"></span>系统管理</li> -->
          <router-link :to="item.subMenuu[0].url" tag="li" active-class="active" class="iconfont" :class="item.icon" v-for="item in this.urlname" @click.native="addhistory" >
            {{item.name}}
          </router-link>
      </ul></el-col>
    ...
    <el-header style="height:auto; overflow:hidden" id="menuContent">
      <div class="submenucontent" style="overflow:hidden; position:relative; padding:18px 0">
...
.topmenu li,.topmentu-right li{display: inline-block; padding:24px 16px; font-size: 16px; color: #fff;}
